...
import { MapPin, Star, ArrowLeft, CalendarDays, X, Shield } from 'lucide-react'
  ...
  return (
    <div className="space-y-12 pb-20 animate-in fade-in slide-in-from-bottom-4 duration-1000">
      ...
      <div className="grid gap-12 lg:grid-cols-[1fr_380px] px-2">
        ...
        <div className="space-y-12">
          ...
          <div className="space-y-8">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">{photographer.portfolio.length} WORK PIECES</span>
            </div>
        ...
        {/* Right Sidebar: Booking Workspace */}
        <div className="space-y-8">
          <div className="rounded-[36px] border border-slate-200 bg-white p-2 shadow-sm sticky top-24 ring-1 ring-slate-100">
            <div className="bg-slate-50/50 p-8 rounded-[32px]">
              <h3 className="text-[11px] font-black uppercase tracking-[0.2em] text-slate-400 mb-8">CHỌN GÓI DỊCH VỤ</h3>
              <div className="space-y-4">
                <Package tier="Standard" price={photographer.startingPrice} desc="1 giờ • 20 ảnh chỉnh màu" />
                <Package tier="Premium" price={Math.round(photographer.startingPrice * 1.6)} desc="2 giờ • 40 ảnh • retouch nhẹ" highlight />
                <Package tier="Deluxe" price={Math.round(photographer.startingPrice * 2.5)} desc="3 giờ • 60 ảnh • retouch pro" />
              </div>
              ...
              {photographer.status === 'APPROVED' ? (
                <button
                  onClick={() => setBookingOpen(true)}
                  className="mt-8 flex w-full h-14 items-center justify-center gap-3 rounded-2xl bg-slate-900 text-[11px] font-black uppercase tracking-widest text-white shadow-xl shadow-slate-900/20 transition-all hover:bg-slate-800 hover:scale-[1.01] active:scale-95"
                >
                  BOOK {photographer.name.split(' ').pop()?.toUpperCase()}
                </button>
              ) : (
      ...
      <BookingModal open={bookingOpen} onClose={() => setBookingOpen(false)} photographer={photographer} />
    </div>
  )
}

function Package({ tier, price, desc, highlight }: { tier: string; price: number; desc: string; highlight?: boolean }) {
  return (
    <div className={`relative flex items-center justify-between gap-4 rounded-2xl border p-5 transition-all cursor-default ${highlight
      ...
      <div>
        <div className={`text-[12px] font-black uppercase tracking-widest ${highlight ? 'text-indigo-100' : 'text-slate-900'}`}>{tier}</div>
        <div className={`text-[11px] font-bold mt-1 ${highlight ? 'text-indigo-200' : 'text-slate-400'}`}>{desc}</div>
      </div>
      <div className="shrink-0 text-right">
        <div className={`text-sm font-black ${highlight ? 'text-white' : 'text-slate-900'}`}>{new Intl.NumberFormat('vi-VN').format(price)}₫</div>
      </div>
      {highlight && <div className="absolute -right-1 -top-1 h-8 w-8 rotate-12 bg-white/10" />}
    </div>
  )
}
